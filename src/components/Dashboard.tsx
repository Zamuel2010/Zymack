...
export default function Dashboard({ user, isDarkMode, toggleDarkMode }: DashboardProps) {
  ...
  // Polling for Tatum Webhook Transactions
  useEffect(() => {
    if (!user) return;

    const syncTatum = async () => {
       try {
          const resp = await fetch(`/api/tatum/sync?uid=${user.uid}`);
          if (resp.ok) {
             const data = await resp.json();
             if (data.transactions && data.transactions.length > 0) {
                // We have new incoming transactions!
                const walletRef = doc(db, 'wallets', user.uid);
                ...
                try {
                   ...
                   console.error("Failed to fetch live rates, using fallback", e);
                   liveRates = {
                      'BTC': 100000000,
                      'ETH': 5000000,
                      'SOL': 250000,
                ...
                for (const tx of data.transactions) {
                   // Calculate fiat value using real rates
                   const rate = liveRates[tx.asset?.toUpperCase()] || 1000;
                   const fiatValue = tx.amount * rate;
                   ...
                   await setDoc(txRef, {
                      ...
                      cryptoAsset: tx.asset,
                      status: 'completed',
                      title: `Deposit ${tx.asset}`,
                      createdAt: new Date().toISOString(),
                      txId: tx.txId || txRef.id
                   });
                }
          ...
          console.error("Tatum sync error", e);
       }
    };

    const interval = setInterval(syncTatum, 5000);
